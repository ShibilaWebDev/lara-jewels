    ...
    <div className={style.home}>
      ...
      <section className={style.hero}>
        <div className={style["hero-text"]}>
          ...
          <button onClick={() => navTo("/collections")}>
            Shop Now
          </button>
        </div>
      ...
      <section className={style["categories-section"]}>
        <h2 className={style["category-heading"]}>
          Collections
        </h2>

        <section className={style["category-grid"]}>
          {categories.map(cat => {
            const imageSrc = cat.imageUrl?.length ? (cat.imageUrl[0].startsWith('http') ? cat.imageUrl[0] : `${envUrl}${cat.imageUrl[0]}`) : "https://via.placeholder.com/400";
            return (
              <div key={cat.category} className={style["category-card"]} onClick={() => navTo(`/collections/${cat.category}`)}>
